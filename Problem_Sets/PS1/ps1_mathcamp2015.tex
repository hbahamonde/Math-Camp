...
\end{bmatrix}
\] Define $x^{\top}\cdot{x}$.
\item Given the transposed vector $v^{\top}_{1\times 4}$
\[v^{\top}=
\begin{bmatrix}
7 & 8 & 9 & 10\\ 
\end{bmatrix}
\] and vector $u_{4\times 1}$
\[u=
\begin{bmatrix}
7 \\
8 \\
...
\end{bmatrix}
\]
find (1) if $v^{\top}_{1\times 4}$ and $u_{4\times 1}$ are conformable and (2) if the product $u_{4\times 1} \cdot v^{\top}_{1\times 4}$ exists. If $u_{4\times 1} \cdot v^{\top}_{1\times 4}$ is defined, find the product.
...
\end{bmatrix}
\]
find (1) the size of each vector, (2) find if the product $v\cdot u$ exists, and (3) find what would be the final size of $v\cdot u$ (if $v\cdot u$ exists). Repeat the same three questions for the relationship $v^{\top}\cdot u$.
\item Given the matrix,
\[x=
\begin{bmatrix}
1 & 2\\
3 & 4\\ 
\end{bmatrix}
\] Find $x^{\top}$. Then (1) define $x^{\top}\cdot x$ and (2) $x \cdot x^{\top}$. Is multiplication commutative?
\item Given the matrix,
\[x=
\begin{bmatrix}
3 & 4\\
5 & 9\\ 
\end{bmatrix}
\] Find $x^{-1}$. To do that, first find the determinant $\begin{vmatrix}x\end{vmatrix}$, then find the adjoint adj$(x)$. This exercise should be handed in \LaTeX.
\end{enumerate}
\end{document}