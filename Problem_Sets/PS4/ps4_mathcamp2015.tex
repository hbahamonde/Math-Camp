...
Choose two exercises to hand it in \LaTeX \; form.

\begin{enumerate}
\item Using the chain rule, find the next derivatives.
  \begin{enumerate}
  \item $(x^{4}-1)^{3}$
  \item $(x^{2}-1)^{4}-6(x+7)^{4}$
  \item $(2x^{3}-6x^{4})^{3} \cdot (4x^{2}-5x)^{2}$
  \end{enumerate}
\item Find both the local and global maxima of these functions.
  \begin{enumerate}
  \item $-3x^{5}+5x^{3}$, where $x\in[-1,2]$
  \item $x^{3}+3x^{2}-24x+3$, where $x\in[-5,5]$
  \end{enumerate}
\item Find the antiderivative $F(x)$ of the following functions, then find the derivatives $f^{\prime}(x)$ of the antiderivatives you just found.
  \begin{enumerate}
  \item $3x^{2}+2x^{3}+1$
  \item $7x^{6}+6x^{5}+x$
  \end{enumerate}
\item Integrate the next functions.
  \begin{enumerate}
  \item $\int_{1}^{4} (5x^{2}-5)dx$
  \item $\int_{1}^{9} \frac{(2x+5)}{2}dx$
  \end{enumerate}
\end{enumerate}
